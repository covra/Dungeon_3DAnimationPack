...
function resetHoverPosition (creatureID)
	if creatureID == ID_CREATURE then		
		SK_CLV_RIGHT:SetRotation (BIND_SK_CLV_RIGHT,3, true)
		SK_CLV_LEFT :SetRotation (BIND_SK_CLV_LEFT,3, true)
		SK_CODO_LEFT:SetRotation (BIND_SK_CODO_LEFT,3, true)
		SK_CODO_RIGHT:SetRotation (BIND_SK_CODO_RIGHT,3, true)
		SK_PELVIS:SetRotation (BIND_SK_PELVIS,3, true)
		SK_CLV_RIGHT:SetRotation (BIND_SK_CLV_RIGHT,3, true)
...
function Tick(dt)
	if ROOT.clientUserData.isFlying then 
		...
		Task.Wait(dt )
	elseif not ROOT.clientUserData.isFlying then 
		--BODY
		--=====
...
Events.Connect("DRG.hold",onAskMov)
